...
Class scrapeJobsHelper extends Helper{
	...
	public function storeData($getParams, $postParams){
		...
		foreach ($listOfAds as $oneCar) {
			$carInfo = $scrapeJobsModel->getJobByUrl($oneCar['adDirectUrl'], ['ad_direct_url']);
			// if(count($carInfo) === 0) {
			// 	$row = [
			...
			// 	];
			// 	// $carId = $scrapeJobsModel->makeInsert('nikitin_ninja.tbl_scraped_cars', $row);
			...
			// 	// 		"sm"     => $oneImage['sm'],
			// 	// 	];
			// 	// 	$scrapeJobsModel->makeInsert('nikitin_ninja.tbl_scraped_cars_images', $imgRow);
			// 	// }
			// 	$newCars++;
			// }
		}
